...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Revisi Temuan Kecelakaan Kerja/Nearmiss
            <div class="pull-right" >
                <h6><a id="btn-op-data-filter" class="" style="text-decoration: underline;" href="#" >  Filter</a></h6>
            </div>
        </h1>
        ...
        <div class="row" >
            <div class="col-md-12" >
                <div id="open-data-filter-box" class="small" >
                    <form name="filter-form" method="POST" action="trans/pnrms/filter-open-data" >
                                ...
                                <table class="table table-condensed" >
                                    ...
                                    <tr>
                                        <td class="text-right" colspan="4" >
                                            ...
                                            <a class="btn btn-xs btn-warning" href="trans/pnrms" >Reset Filter</a>&nbsp;
                                        </td>
                                    </tr>
                                </table>  
                            </div>
                            <div class="box-footer text-right" >                                
                                <a id="btn-op-data-close-filter" class="" href="#" style="text-decoration: underline;" ><i class="fa fa-angle-double-up"></i>  Close Filter</a>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <div class="col-md-8" >
                <div class="box box-danger"  >
                    ...
                    <div class="box-body" style="min-height: 445px;max-height: 445px;" >
                        ...
                        <div id="table-op-data" ></div>
                    </div>
                    <!-- /.box-body -->
                    <div class="box-footer " id="open-data-footer" data-tableid="table-op-data" ></div>
                </div>
            </div>
            <div class="col-md-4" >
...
//        var parentId = $(this).parent().parent().parent().attr('id');
        ...
        $.get(url, null, function (res) {
            ...
            if (parentId == 'table-op-data') {
                $('#open-data-footer').empty();
    ...
    //filter open data
    $('#btn-op-data-filter').click(function () {
//        $('#btn-op-data-close-filter').fadeIn(0);
        $('#btn-op-data-filter').hide();
    ...
    //close filter
    $('#btn-op-data-close-filter').click(function () {
        $('#open-data-filter-box').slideUp(250, null, function () {
//            $('#btn-op-data-close-filter').hide();
        });
        $('#btn-op-data-filter').fadeIn(100);
    ...
    $('form[name=filter-form]').submit(function () {
        ...
        if (status == 'OP') {
            //load data
            $('#table-op-data').overlay();
            var url = 'trans/pnrms/open-data/' + 'true' + '/' + tglAwal + '/' + tglAkhir + '/' + kolom + '/' + filter_text;
            $.get(url, null, function (res) {
                $('#table-op-data').html(res);
                //pindahkan paging ke footer
                $('#open-data-footer').empty();
                var pager = $('#table-op-data').find('.pagination');
                pager.detach().appendTo('#open-data-footer');
                pager.addClass('pagination-sm no-margin pull-right');
                $('#table-op-data').overlayout();
            });
        } else if (status == 'CK') {
            $('#table-checked-data').overlay();
            var url = 'trans/pnrms/checked-data/' + 'true' + '/' + tglAwal + '/' + tglAkhir + '/' + kolom + '/' + filter_text;
            $.get(url, null, function (res) {
...
//    $('#table-op-data').overlay();
//    $.get('trans/pnrms/open-data', null, function (res) {
//        $('#table-op-data').html(res);
//        $('#table-op-data').overlayout();
//    });
//}
...
//    $('#table-checked-data').overlay();
//    $.get('trans/pnrms/checked-data', null, function (res) {
//        $('#table-checked-data').html(res);
//        $('#table-checked-data').overlayout();
//    });
//}
